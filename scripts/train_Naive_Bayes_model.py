...
def load_data(file_path):
    ...
    for item in data:
        ...
        labels.append(item['label'])  # Updated to use 'label'
    return texts, labels
...
def evaluate_model(model, X_test, y_test, label_encoder):
    ...
    y_test_decoded = label_encoder.inverse_transform(y_test)  # Decode test labels
...
def main():
    ...
    model = train_naive_bayes_with_cv(X_train, y_train_encoded, cv_folds=5)
